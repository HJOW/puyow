...
TIME_MULTIPLIER_LOG2_SCALE = 12.0
...
# 관측 벡터가 자기 필드만 담아 승패(WIN_REWARD/LOSS_REWARD)를 상태로 설명할 수 없으므로, 감가율이
# 높으면 그 큰 항이 한 판 내내 감쇠하지 않고 모든 상태로 퍼져 "공격해서 이기기" 대신 "보드를 낮게
# 유지해 버티기"를 배운다. 1500 에피소드 학습 후 승률 측정에서 0.99는 학습 전보다 나빴고 값을
# 낮출수록 개선되어 0.70~0.80 구간에서 최선이었다(자세한 측정은 INFO_FOR_AI.md의 인수인계 절 참고).
# 0.70/0.75/0.80을 시드 둘씩 재현한 평균에서 0.70이 근소하게 가장 좋고 시드 간 편차도 가장 작았다.
DISCOUNT_GAMMA = 0.70
