...
기본 제공 `PuyoW.Vapula`(바퓰라)와 `PuyoW.Oriax`(오리아스)는 자간 다음 순서의 출시 예정 적입니다. 두 클래스는 `OnnxEnemy`를 상속하며, 임시로 자간과 동일한 판단과 `onnx/model03.onnx` 모델을 사용합니다. 전용 모델을 준비하면 각 생성자의 `modelPath`를 독립적으로 교체할 수 있습니다.

`notAvail = true`이므로 적 선택 화면에서는 회색 출시 예정 카드로 표시되며 선택·구경·카드 획득 대상에서 제외됩니다. ONNX 런타임이 없는 페이지에서는 다른 ONNX 적처럼 선택 목록에서 숨깁니다. 갤러리에는 기존 잠금 규칙으로 등록됩니다. 초상화는 기존 Canvas 인간형 스타일의 일반·위기·패배 표정을 사용합니다.
