...
`lookaheadSearchMode = 'advanced'`이면 Worker의 빠른 보드·빔 탐색·발화점 평가·방해뿌요 도착 예측을 사용합니다. 기본 제공 적에서는 자간·바퓰라(목표 5연쇄), 오리아스·아미(6), 오세·그레모리·오로바스(7)가 `RealtimeLookaheadEnemy`를 상속합니다. 작은 연쇄 점등은 모두 켜며, 탐색 수 3·시간 50ms·빔 폭 5를 공유합니다. 외부 적은 `PuyoW.Enemy`와 Worker 탐색 보조 함수를 사용하세요.
