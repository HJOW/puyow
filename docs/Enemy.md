...
기본 제공 `PuyoW.Vapula`(바퓰라)와 `PuyoW.Oriax`(오리아스)는 자간 다음 순서의 적입니다. 두 클래스는 `OnnxEnemy`를 상속하며 `onnx/model03.onnx` 모델을 사용합니다. 전용 모델을 준비하면 각 생성자의 `modelPath`를 독립적으로 교체할 수 있습니다. 초상화는 기존 Canvas 인간형 스타일의 일반·위기·패배 표정을 사용합니다.

바퓰라는 BUILDNO 81에 출시되어 자간을 이긴 뒤 선택할 수 있고, 구경 모드 후보에서는 다른 ONNX 적처럼 빠지며 카드 획득 대상에 들어갑니다. 오리아스는 `notAvail = true`인 출시 예정 적이라 적 선택 화면에서는 회색 출시 예정 카드로 표시되며 선택·구경·카드 획득 대상에서 제외됩니다. 둘 다 ONNX 런타임이 없는 페이지에서는 다른 ONNX 적처럼 선택 목록에서 숨기고, 갤러리에는 기존 잠금 규칙으로 등록됩니다.

BUILDNO 112에서 오리아스 다음에 아미(`Amii`)·오세(`Ose`)·그레모리(`Gremory`)·오로바스(`Orobas`)·무르무르(`Murmur`)·카임(`Caim`)·알로케스(`Alokes`)·발람(`Balaam`)·푸르카스(`Purkas`)를 순서대로 추가했습니다(`sortPriority` 15~23). 모두 `PuyoW`의 공개 클래스이며, `OnnxEnemy`를 직접 상속합니다. 출시 상태·임시 모델 경로·선택 제한은 오리아스와 같습니다. 갤러리에는 기존 잠금 규칙으로 등록되고, 카드·구경·리더보드 적 목록·Python 학습 상대에는 들어가지 않습니다. 실제 출시 시 각 생성자의 `notAvail`과 `modelPath`를 변경합니다.

각 적의 일반·위기·패배 초상화와 의상에 맞는 필드·베젤·중앙 배경 테마를 구현했습니다. 이름과 공개 클래스 철자는 프로젝트 표기를 사용합니다. [전승 참고 자료](https://en.wikipedia.org/wiki/List_of_demons_in_the_Ars_Goetia)의 동물·소품은 기존 머리 하나의 인간형 Canvas 그림체에 맞게 장식으로 재해석했습니다.

| 적 | 초상화의 주요 상징 | 테마 |
| --- | --- | --- |
| 아미 | 불꽃 머리 장식·불꽃 망토·별무늬 책 | 적동색 |
| 오세 | 표범 귀·꼬리·가면·왕관 | 올리브색 |
| 그레모리 | 긴 머리·허리의 관·낙타 문양 보석함 | 장밋빛 |
| 오로바스 | 말 귀·갈기·말발굽 방패 | 강청색 |
| 무르무르 | 공작의 관·깃털 날개·나팔·혼불 | 회녹색 |
| 카임 | 검은 새 깃털·검·불씨 | 회자색 |
| 알로케스 | 사자 갈기·별 문장 갑옷·기병창 | 벽돌색 |
| 발람 | 곰 귀·소와 양의 뿔 왕관·매 문장·뱀 꼬리 | 황토색 |
| 푸르카스 | 백발·수염·갈래창·철학서 | 청회색 |
...
단색 배경만 쓸 것이라면 세 메서드 대신 `getFieldThemeColors()` 하나만 재정의하는 편이 간단합니다. 이 메서드는 `{ bezel, field, center }` 형태의 색 문자열을 반환하며, 위 세 메서드의 기본 구현이 그대로 사용합니다. `field`(플레이 영역 내부)는 `bezel`보다 밝게, `center`(중앙 영역)는 가장 어둡게 두는 것이 기본 제공 적들의 규칙입니다. `center` 색은 베젤 바깥의 화면 여백에도 함께 깔리므로, 이 값을 바꾸면 게임 화면 전체의 바탕색이 바뀝니다.

기본 제공 적 중 솔로몬과 연습·연속 피버·퍼즐뿌요·플레이 방법이 쓰는 내부 상대는 기본 테마를 사용하고, 나머지 기본 제공 적은 각자 초상화 색과 어울리는 테마를 가집니다. 구경 대전에서는 두 CPU가 모두 적이므로 양쪽 필드와 중앙 영역이 모두 우측 CPU의 테마를 사용합니다.
...
독립 3D 버전의 AI는 제공하지 않습니다. 2D 게임의 선택적 연출이나 외부 분석 도구가 같은 후보 계산을 읽어야 한다면 `PuyoW.common.findLandingPlacement()`, `estimateCombo()`, `estimateAttack()`, `findBestPreviewResult()` 및 `simulateNMovePlacements(player, targetCombo, turnCount)`를 사용할 수 있습니다. 기본 룰·피버 룰 대전(구경 포함)에서는 `player.nextPairs`에 현재 수 뒤 20쌍이 미리 확정되어 있고, 동기 N수 탐색과 Worker 탐색은 이 전체 대기열을 사용합니다. 중앙 화면과 `PuyoW.getNextPairs()`는 기존처럼 앞 두 쌍만 보여 줍니다.
...
브라우저 ONNX 추론 적(`OnnxEnemy` 계열: 발라크·자간·바퓰라·오리아스)도 BUILDNO 87부터 같은 방식으로 실시간 반응합니다. BUILDNO 90의 모델 버전 4부터는 **모델이 실시간 상황을 직접 봅니다.** 관측값(1035개)에 상대 필드 전체와 함께, 진행 중인 상대 연쇄의 아직 확정되지 않은 공격·그 공격이 떨어지기 전에 둘 수 있는 배치 수·상대 연쇄 진행 여부가 들어갑니다. 받을 피해량 자리에는 예전처럼 예측을 섞지 않고 확정 DAMAGE만 넣습니다. 추론 결과로 조작하는 동안 이 상태가 바뀌면 지금 도달할 수 있는 위치만으로 다시 추론하며, 빠른 하강 시작 뒤·기준(기본 룰 `ignorableIncomingGarbage` 4, 피버 룰 1) 미만·적 자신의 피버 중·연속 피버에서는 다시 추론하지 않습니다. 재추론이 실패하거나 마감 시한을 넘기면 직전 배치를 유지합니다. `realtimeReaction = false`로 끌 수 있습니다.

모델 버전 4의 관측 계약은 버전 3과 호환되지 않습니다. 예전에 내보낸 `.onnx` 파일을 그대로 두면 입력 길이가 달라 추론할 수 없으므로, 게임은 세션을 빌릴 때 입력 길이를 확인해 다르면 한 번 알리고 그 대전을 기존 시뮬레이션 인공지능으로 진행합니다.
...
// 뿌요가 이미 착지했거나 턴이 바뀌었다면: request.cancel('contact');
```
