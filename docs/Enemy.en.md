...
## Released opponents and AI assignments

BUILDNO 113 releases every opponent through Alokes. Balaam and Purkas remain unavailable (`notAvail = true`) with no implemented AI, ONNX dependency, or model path. They retain their gallery entries and gray upcoming cards, including on pages without the ONNX runtime.
...
| --- | --- |
| Decarabia | Decarabia, Belial |
| Belial | Amdusias, Kimaris |
| Amdusias | Andrealphus, Flauros |
| Kimaris | Andras, Valak |
| Andrealphus (target 5) | Zagan, Vapula |
| Flauros (target 6) | Oriax, Amii |
| Andras (target 7) | Ose, Gremory, Orobas |
...
Set `lookaheadSearchMode = 'advanced'` on an opponent to make `PuyoW.startWorkerLookaheadSearch()` use the advanced search inside the Worker. Without it, the existing search above is used unchanged. The built-in opponents Zagan through Orobas use this mode. All of the advanced-search and real-time re-planning logic below lives in `PuyoW.RealtimeLookaheadEnemy`, a common class for built-in opponents; Zagan inherits it and only sets its target chain (`targetCombo`) and whether to light lamps with small chains (`lightFeverGaugeWithSmallChains`) via `super({ targetCombo: 5, lightFeverGaugeWithSmallChains: true })`. Zagan and Vapula target 5 chains, Oriax and Amii target 6, and Ose, Gremory, and Orobas target 7. All enable small-chain lamp lighting. Because this class inherits the built-in-only `BundledEnemy`, external opponents should keep using `PuyoW.Enemy` and the Worker search helpers as in the example above.
