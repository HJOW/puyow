...
Browser ONNX inference enemies (the `OnnxEnemy` family: Valak, Zagan, Vapula, Oriax) also react in real time from BUILDNO 87. The model input (528 observation values) is unchanged; the incoming-damage slot receives the settled DAMAGE plus the predicted attack of the opponent's ongoing chain (in the Fever rule's normal state, only the groups that can be offset now). While moving by an inference result, if that amount or the opponent's chain state changes, the enemy re-runs inference over the placements reachable from its current position. It does not re-infer after fast drop has started, when the amount stays below the threshold (standard rule `ignorableIncomingGarbage` 4, Fever rule 1) both before and after, while the enemy itself is in Fever, or in Continuous Fever. If a re-inference fails or passes the deadline, the previous placement is kept. The model cannot see how many placements later the garbage will land. Set `realtimeReaction = false` to turn it off.
