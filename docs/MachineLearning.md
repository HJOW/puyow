...
`npm start`로 띄우는 [nodeserver/server.js](../nodeserver/server.js)도 같은 `/apis/localmodelinfo`·`/v1/chat/completions` 계약을 제공하므로, 파이썬 없이도 게임 설정에서 **Local AI**를 골라 솔로몬과 대전할 수 있다. 이 서버는 `.pt` 대신 [src/onnx/default.onnx](../src/onnx/default.onnx)를 `npm install`로 설치되는 `onnxruntime-node`로 추론하며, 모델 경로는 `nodeserver/server.js`의 `LOCAL_AI_MODEL_PATH` 상수로 바꾼다. 이 경로에 파일이 없으면 `/apis/localmodelinfo`가 `available: false`를 돌려주어 게임에서 Local AI를 고를 수 없고, 정적 파일과 다른 API는 그대로 동작한다. 배치를 고르는 규칙(애프터스테이트 보상 + 0.70 × 가치)은 `pythonserver.py`와 같다. 역학습은 지원하지 않아 `/apis/solomonlearning`은 요청을 받기만 하고 모델을 바꾸지 않는다.
