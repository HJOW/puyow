...
AI 제공자를 **Local AI**로 두고 **극한** 난이도로 적 **솔로몬**과 대전하면(색상 수와 룰은 가리지 않는다), 그 대전에서 솔로몬이 둔 수로 서버에 로드된 모델을 추가 학습한다. 솔로몬의 배치는 원래도 이 서버의 DQN이 정하므로, 사람과 실제로 겨룬 결과를 그대로 그 모델에 되먹이는 셈이다.
...
학습 결과는 게임의 브라우저 콘솔에 `솔로몬 학습 적용 결과`로 기록되며, 반영한 전이 수와 마지막 손실값을 확인할 수 있다. 이 요청이 실패하더라도 게임 진행에는 영향을 주지 않는다.
