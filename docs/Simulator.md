...
메인 메뉴의 **시뮬레이터**에서 팔레트를 선택해 필드에 뿌요를 배치합니다. **재생**으로 중력과 연쇄 결과를 확인하고, 완성한 배치는 **JSON복사**로 내보냅니다. **초기화**를 누르면 좌측 플레이 영역의 배치가 모두 제거됩니다. 팔레트의 `철구뿌요`는 시뮬레이터에서만 사용할 수 있는 고정 장애물입니다. 폭발하거나 다른 뿌요 폭발의 영향을 받지 않으며 점수에도 포함되지 않습니다. 시뮬레이터의 점수 계산은 실제 게임과 같은 식을 사용합니다.
...
ATTACK 증가량 = (점수 증가량 / 마진 레이트) * EXPLOSION_REWARD_MULTIPLIER * 시간 진행 배율
```

연결 보너스는 색별 연결 그룹마다 따로 더하지 않습니다. 단색 폭발이면 그 색으로 사라진 일반 뿌요 전체 수를, 여러 색이 동시에 폭발하면 색별 수를 합산한 뒤 가장 많은 한 색의 수를 한 번 계산합니다. 예를 들어 빨강 4개와 파랑 5개가 동시에 폭발하면 일반 뿌요 수는 9개, 색수 보너스는 2색 기준 3, 연결 보너스는 가장 많은 파랑 5개 기준 2입니다.

`src/js/puyow.js`의 `HARD_GARBAGE_SCORE_MULTIPLIER`는 현재 `2`입니다. 따라서 1연쇄에서 보너스 값이 1인 일반 뿌요 4개가 폭발할 때, 딱딱뿌요 1개를 함께 파괴하면 `4 * (1 * 2 + 1) * 1 * 10 = 120`점이고, 2개를 함께 파괴하면 `4 * (2 * 2 + 1) * 1 * 10 = 200`점입니다.

시간 진행 배율은 실제 게임 경과 시간이 300초까지는 1이며, 320초부터 20초마다 두 배가 되어 최대 1024입니다. 게임·AI 예상 공격은 이 값을 적용하고, 시간 개념이 없는 시뮬레이터는 기본값 1을 사용합니다. 이 상수와 계산식은 실제 게임, 시뮬레이터, AI의 예상 공격 계산이 공유합니다. 값을 바꾸면 `tests/test01_simulator.spec.js`의 딱딱뿌요 1개·2개 동시 파괴 점수 기대값도 같은 식으로 갱신해야 합니다.
