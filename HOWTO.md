...
window.addEventListener('puyow_prerender', (event) => {
    // 이전 화면을 지운 직후이므로 게임 배경보다 먼저 그릴 수 있습니다.
...
`convertURL(url)`은 상대경로와 절대 URL 모두에서 `[CTX]`를 컨텍스트 경로로, `[LANG]`를 설정 화면에서 고른 게임 언어의 두 글자 코드로 치환합니다. 지원하는 언어는 `en`, `ko`, `ja`, `zh`, `fr`, `de`입니다. 언어를 저장한 적이 없으면 브라우저 시스템 언어를 따르고, 시스템 언어나 저장된 언어가 지원하지 않는 언어이면 `en`을 사용합니다.
