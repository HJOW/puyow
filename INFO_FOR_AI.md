...
### 텍스트 입력 대화상자 (2026-09-15, BUILDNO 58)

- 공개 `askText(message, multiline = false)`는 초기화된 캔버스 위에 음영 처리된 텍스트 입력 대화상자를 표시하고, 입력 완료 시 문자열을 `Promise`로 반환하며 ESC·취소 시 `null`을 반환한다. `message`는 원문 그대로 표시하고 확인·취소 버튼은 `translate()`를 거친다. `multiline`이 `true`이면 입력창·확인·취소의 세 포커스를 방향키로 옮기며, 입력창에서 Enter를 눌러 입력 모드로 전환한 뒤 Enter는 줄바꿈으로 사용한다. 입력 모드의 ESC는 입력 모드를 해제하고, 포커스된 확인·취소 버튼은 Enter로 선택한다. 기본값·`null`은 한 줄 입력이며, 문자열이 아닌 `message`나 boolean·null·생략 이외의 `multiline`은 예외다.
