...
- 메인 메뉴의 게임 시작은 `적과 대전`·`스스로 연습`·`퍼즐뿌요`·`취소`를 먼저 보여 준다. 적과 대전은 기본 룰·피버 룰·피버 룰 (시작), 스스로 연습은 연습·연속 피버를 보여 주며 두 하위 단계에도 취소가 있다. 퍼즐뿌요는 스테이지 선택으로 바로 이동한다. 두 단계 모두 선택지 바깥 클릭은 무시하고, ESC는 해당 단계의 취소와 같다. 첫 단계 취소는 메인 메뉴, 둘째 단계 취소는 첫 단계로 돌아간다. 잠긴 피버 룰 (시작)은 포커스·활성화 대상에서 제외한다.
