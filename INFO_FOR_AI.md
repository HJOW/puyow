...
## 인수인계: 갤러리 카드 등장 3D 효과 (2026-09-14, BUILDNO 55)

### 완료한 작업

- `TODO.md`의 카드 뽑기·합성 결과 등장 연출을 구현했다. 확인창 승인 후 기존 GOLD 차감·카드 소비·결과 저장을 마친 다음에만 연출한다. 효과 유무나 오류 때문에 결과를 다시 추첨하거나 자원을 다시 차감하지 않는다.
- 실제 THREE 렌더러·카드 텍스처·입체 회전·광륜·광채·입자·GPU 자원 관리는 전부 `src/js/puyow_3d.js`에 있다. `src/js/puyow.js`는 기존 카드의 2D 그림과 등급 정보를 전달하고 초기화·프레임·입력·정리 접점을 연결한다. 기본 카드 목록과 효과의 그림은 `drawCardTarget()`을 공유한다.
- COMMON/UNCOMMON/RARE/EPIC별 연출 시간은 2,200/2,650/3,100/3,550ms다. 여러 장은 한 시간축의 격자에 함께 등장하며, 시간은 그 묶음의 최고 등급을 따른다. 카드별 광륜·입자·회전·광채는 각 카드 자신의 등급을 따른다. 카드 등급 문구는 추가하지 않았다.
- 렌더러는 첫 연출에서 지연 생성한다. THREE 또는 효과 모듈이 없거나 WebGL 생성·렌더링·컨텍스트 유지가 실패하면 2D 화면으로 계속 진행한다. WebGL 오류가 발생한 매니저는 `failed`로 남겨 반복 실패를 막고, 게임을 재초기화하면 다시 시도할 수 있다.
- 연출의 Enter·Space·ESC·게임패드 확인/취소·클릭은 연출만 건너뛴다. 방향키·휠은 배경 카드 목록을 조작하지 않는다. 3D 캔버스는 앞에 있어도 `pointer-events: none`이며, 기존 2D 캔버스의 회전 좌표 변환과 클릭 처리를 유지한다.
- 종료 타이머와 절대 RAF 시각을 함께 사용한다. 초기 렌더에서 이미 실패하거나 종료했으면 타이머를 새로 남기지 않는다. 완료·취소 때 geometry/material/texture와 렌더 목록을 해제하고, `destroy()` 때 렌더러·이벤트도 해제한다.
- `webpack.config.js`의 진입 목록에 효과 모듈을 추가하고 `src/bundle/puyow.bundle.js`를 재생성했다. BUILDNO는 54에서 55로, `package.json` 버전은 `0.0.55`로 변경했다. 버전값 자체를 검증하는 테스트는 추가하거나 실행하지 않았다.

### 테스트와 확인 결과

`tests/test01_menu.spec.js`의 `카드 3D 등장 연출` 그룹에 7개 테스트를 추가했다. 실제 GPU 렌더 호출·픽셀, 시간 제한·자원 해제, EPIC 10장 동시 표시, 합성·게임패드 건너뛰기, THREE/효과 모듈/WebGL 각각의 부재, 프레임 지연·GPU 손실·destroy/재초기화·렌더 오류를 검증한다. 기존 카드 지급·확인창·캔버스 초기화·화면 회전 테스트도 함께 실행했다.

최종 실행 명령은 다음과 같으며 **Chromium 14개 모두 통과했다**.

```powershell
npx.cmd playwright test tests/test01_menu.spec.js tests/test01_core.spec.js --project=chromium --grep '카드|Three.js|초기화는 최상위|세로 화면|가로 화면' --workers=1 --reporter=line
```

`node --check src/js/puyow.js`, `node --check src/js/puyow_3d.js`, `npm.cmd test`(ESLint), Webpack 빌드, `git diff --check`도 통과했다. Playwright 웹 서버의 `npm start`가 ESLint와 빌드를 수행했다. 일반 카드·EPIC 10장·세로 화면 캡처를 직접 확인했고, 최초 캡처에서 발견한 카드 뒷면의 앞면 가림은 두 면을 같은 그룹으로 회전하도록 수정했다. 캡처는 Git에서 제외되는 `test-results/`에 있다.

### 다음 작업자가 참고할 범위

- 브라우저 검증은 `src/puyow.html`의 개별 스크립트 로드 경로를 사용했다. Firefox·WebKit, 실제 모바일 GPU, 번들만 불러오는 페이지의 실행은 이번 작업에서 확인하지 않았다. 번들은 빌드 성공까지 확인한 상태다.
- 레이어나 효과를 수정할 때 2D 입력 경로, THREE 없는 실행, 실제 시간 제한, 카드 지급의 단일 실행을 함께 유지한다. 카드 앞면과 두께를 따로 회전시키면 뒷면이 앞면 일부를 가릴 수 있으므로 `body` 그룹을 함께 회전시킨다.
- `TODO.md`에 사용자가 적은 요청은 그대로 보존했고 커밋은 만들지 않았다. 구현·테스트 작성·인수인계 기록을 마쳤으며 사용자 요청에 따라 이 지점에서 작업을 중단한다.
