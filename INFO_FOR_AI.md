...
- 피버 전용 필드와 연속 피버 필드의 **고정된 보드 뿌요** 중력은 일반보다 1.5배 빠르다. 컨트롤 단계에서 새로 지급된 뿌요 쌍의 자연 낙하에는 이 배율을 적용하지 않으며, 자연 낙하 속도는 게임 경과 1분마다 0.2배씩 증가해 `MAX_PLAYER_FALL_SPEED_MULTIPLIER`(현재 256배, 1275분 경과)에서 멈춘다.
...
- 보유 카드는 `puyow_cards`의 개별 `{id,type}` 인스턴스 배열이다. 기본 카드 풀은 일반뿌요 5종, 방해뿌요 3종, 예고뿌요 11종(단위 1·6·30·210·500·2000·80000·500000·3000000·20000000·140000000), 출시 적 11종(안드로말리우스~발라크)이며 `getCardDefinitions()`의 가중치를 쓴다. 안드라스·발라크를 포함해 플라우로스 뒤에 추가된 출시 적은 자동으로 가중치 1 카드가 되며, 솔로몬과 `notAvail` 출시 예정 적은 제외한다. 단위 500000보다 큰 예고뿌요도 자동으로 가중치 1 카드가 된다. 카드 배경색은 가중치 경계에 따른 EPIC/RARE/UNCOMMON/COMMON 색을 사용하며 등급 문구는 카드에 표시하지 않는다.
...
- 공개 `setGameElapsed(elapsed)`는 진행 중인 게임의 경과 시간을 지정한 값으로 옮기고 마진 레이트·시간 진행 배율을 다시 계산한다. 조작 뿌요의 자연 낙하 속도(`getPlayerFallSpeedMultiplier()`, 1분마다 0.2배씩 늘어 1275분 경과 시 상한 256배)도 경과 시간에서 파생되므로, 긴 대전의 후반 상황을 실제로 기다리지 않고 재현할 때 사용한다. 진행 중인 게임이 없거나 0 미만·유한하지 않은 값이면 예외를 던진다.
...
## 인수인계: 피버 예고뿌요 표시 수정 (2026-09-16, BUILDNO 73)

### 사용자 지시와 현재 상태

- 사용자가 사용량 한도 때문에 **구현과 테스트 코드 작성까지만 수행하고, 테스트 실행 없이 인수인계 후 중단**하도록 요청했다. 다음 작업도 실행 허가를 받기 전에는 테스트를 자동으로 재개하지 않는다.
- 이 지시 전에 Chromium 테스트를 시작했지만, 지시 수신 즉시 실행 세션에 Ctrl+C를 보내 종료했다(종료 코드 1). 첫 테스트 시작 로그까지만 확인했으며 완료·통과 결과는 없다. 이후 추가 테스트·정적 검사는 실행하지 않았다. 아래 테스트 코드는 중단 이후에도 보완했으므로 현재 상태는 미검증이다.
- 이번 변경 전 BUILDNO 72에서 통과한 테스트 결과를 이번 수정의 검증 결과로 취급하면 안 된다. 커밋하지 않았다.

### 원인과 구현 내용

- `WARN_REPLAY.json`은 BUILDNO 72의 완화 피버 구경 기록이다. 28.632초 좌측 연쇄 시작 후 28.799초 우측이 피버에 진입했다. 해당 공격은 일반 DAMAGE 대상인데, 미정산 예고가 앞쪽에 보이다가 34.115초 최종 정산에서 124가 일반 DAMAGE로 들어가며 흐린 뒤쪽으로 이동해 보였다.
- 피버 스테이지 전환 자체는 피해를 옮기지 않았다. 48.248초 전환에서 피버 DAMAGE 1·일반 DAMAGE 17, 57.281초 전환에서 피버 DAMAGE 4·일반 DAMAGE 13이 각각 유지됐다. 문제는 실제 정산이 아니라 미정산 예고의 목적지 미분리였다.
- `src/js/puyow.js`: `normalWarningPreview()`·`currentFieldWarningAmount()`를 추가했다. 현재 예고를 생성한 에너지의 `targetFeverId`를 현재 수신자 피버 회차와 비교해 일반 필드행 예고를 뒤쪽으로 분리한다. 에너지가 소멸하거나 다음 연쇄가 시작되어도 현재 표시를 소유한 `announcedAttackEnergy`를 기준으로 한다. 실제 DAMAGE 정산·상쇄·AI 판단 함수는 그대로다.
- `drawPlayer()`의 앞쪽 예고는 분리 후 수치, 뒤쪽 예고는 일반 DAMAGE + 일반 필드행 미정산 예고를 사용한다. 뒤쪽 투명도 0.3·X 오프셋 -8은 유지한다. 공개 상태의 `warningPuyos`도 앞쪽 표시와 맞췄다.
- 리플레이는 수신 플레이어 델타의 선택 필드 `nw`를 추가해 일반 필드행 미정산 예고량을 기록·복원한다. 재생용 에너지는 표시 전용이라 피해 목적지를 복원하지 않는다. 형식 버전은 3 그대로이며, `nw` 없는 구형 리플레이는 예전 표시를 유지한다. **기존 WARN_REPLAY.json의 화면까지 소급 수정한 것은 아니며 원본도 변경하지 않았다.**
- BUILDNO는 73, `package.json` 버전은 `0.0.73`으로 갱신했다. 이 버전값은 테스트하지 않는다. 중단 전 Playwright 서버 준비 과정에서 webpack이 실행되어 번들도 갱신됐지만, 최종 검증 완료를 의미하지 않는다.

### 작성한 테스트와 다음 확인 사항

- `tests/test01_fever_damage.spec.js`의 기존 5개 사례에 실제 예고뿌요 `draw()` 호출의 위치·투명도·수량 관찰을 추가했다. 일반 필드행 예고가 정산 전후 모두 뒤쪽에 남는지(즉시/지연 정산), 피버행 예고가 앞쪽에 보이고 일부 상쇄 후 다음 피버 스테이지에서도 유지되는지를 검사한다.
- 추가 사례 2개: 일반 필드행 미정산 공격이 전량 상쇄되면 뒤쪽 예고도 사라지는지, 실제 기록기의 `nw`가 새 리플레이의 앞뒤 표시를 보존하고 `nw` 없는 구형 기록도 기존 방식으로 재생되는지 검사한다. 리플레이 사례는 실제 기록 델타를 합친 짧은 장면을 사용하며 사용자 파일에 의존하지 않는다.
- **현재 7개 테스트는 실행 검증 전이다.** 특히 가상 시계와 예고 도착/정산 경계, 지연 정산 직후 0 검사, 피버 스테이지 전환 대기, 리플레이 입력 중 시계 재개·일시정지 및 메뉴 복귀를 확인해야 한다. 렌더 관찰은 마지막 프레임의 오른쪽 예고를 집계한다.
- 사용자 허가 후 권장 순서: 새 spec의 Chromium 실행 → Firefox/WebKit → 기존 피버·상쇄·리플레이 회귀 → ESLint·문법·diff 검사 및 필요 시 번들 재생성. 서버 포트 9891을 공유하므로 별도 Playwright 실행을 동시에 띄우지 않는다.
- 이번 턴 시작부터 존재한 TODO.md, BUILDNO 72의 DAMAGE 귀속 수정, INFO_FOR_AI.md·package.json·번들 변경과 새 테스트 파일을 보존했다. `WARN_REPLAY.json`도 사용자의 미추적 파일이므로 삭제·덮어쓰지 않는다.

### 재개 후 검증 완료

- Chromium·Firefox·WebKit에서 `tests/test01_fever_damage.spec.js` 7개씩 총 21개가 통과했다. 일반 필드행 미정산 예고의 뒤쪽 표시(즉시·지연 정산), 피버행 예고의 앞쪽 표시와 다음 피버 스테이지 유지, 전량 상쇄 시 뒤쪽 예고 제거, 새 `nw` 리플레이 표시 보존 및 `nw` 없는 구형 형식 3 재생을 실제 캔버스 draw 호출로 확인했다.
- Chromium에서 기존 피버 21개, 피버 리플레이 1개, 공개 게임 상태 2개, 너랑 나랑 피버 시작 1개로 25개가 통과했다. 합계 브라우저 회귀는 46개다.
- `node --check src/js/puyow.js`, `npm.cmd test`, 변경 파일 대상 `git diff --check`, 테스트 모듈 문법 검사가 통과했다. `npm.cmd run webpack`은 제한 환경의 ESLint 캐시 `EPERM`으로 처음 한 번 실패했지만 권한 승격 재실행에서 성공하여 `src/bundle/puyow.bundle.js`를 갱신했다.
- 이 인수인계의 미검증 상태와 테스트 미실행 지시는 중단 당시의 기록이다. 위 재개 후 검증으로 해소됐으며, 현재 작업은 완료 상태다.
