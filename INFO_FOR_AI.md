...
- `BigBangWarningPuyo`는 단위 500000, type `big-bang`인 한 칸 크기의 빅뱅 예고뿌요다. 큰 단위부터 정렬된 `WARNING_PUYO_CLASSES`에서 펜터렉트·테서렉트 다음 세 번째 항목으로 공격량 분해·갤러리·카드 그림에 함께 쓰인다.
...
- `PenteractWarningPuyo`는 단위 20000000, type `penteract`인 한 칸 크기의 펜터렉트(5차원 초입방체) 예고뿌요이며 현재 가장 큰 단위다. `WARNING_PUYO_CLASSES`의 첫 항목이라 공격량을 가장 먼저 가져가고 남은 만큼을 테서렉트 이하가 채운다. 테서렉트와 마찬가지로 빅뱅보다 큰 단위라 가중치 1 카드·갤러리·메인 메뉴 연출에 자동으로 들어간다.
- 두 초입방체 예고뿌요의 그림은 좌표를 손으로 적지 않고 계산한다. 공통 도우미 `createHypercubeGraph(dimensions)`가 꼭짓점 2^n개와 축 하나만 다른 모서리를, `rotateOnPlane()`이 한 평면 회전을, `normalizeProjectedShape()`가 셀 중앙 정렬·반지름 1 정규화·깊이 0~1 환산을 맡는다. 새 차원의 예고뿌요를 더할 때도 이 셋을 재사용한다.
...
- 두 예고뿌요의 투영 결과는 모듈 적재 때 한 번만 만들어 `TESSERACT_PROJECTION`·`PENTERACT_PROJECTION`에 담아 매 프레임 재사용한다.
