...
- BUILDNO 85부터 `drawFieldBezelForeground()`의 클립이 베젤 상단 한 칸 대신 화면 맨 위(`y = 0`)부터 `FIELD_TOP`까지를 덮는다. 패배 연출의 `drawDefeatAnimation()`은 `startDefeatSequence()`가 담은 보드 25줄 전체를 상단 제한 없이 그리는데, 숨김 영역 13번째 줄 이상은 낙하 시작 위치가 `FIELD_TOP - CELL`보다 위(13줄은 `y = 26`, 14줄은 `y = -12`)여서 예전 클립으로는 가려지지 않아 베젤 위로 튀어나와 보였다. 넓힌 베젤 영역은 `{ x: fieldX - CELL, y: 0, width: CELL * 8, height: FIELD_BOTTOM + CELL }`이다. 플레이어 이름(`y = 54`)·예고뿌요·피버 게이지는 모두 이 함수보다 나중에 그려지므로 그대로 보인다. 좌우 필드 베젤의 x 범위는 150~454, 826~1130으로 겹치지 않아 넓힌 베젤이 상대 필드나 중앙 영역을 침범하지 않는다. `drawBezelBackground()`를 재정의해 그라데이션을 쓰는 외부 테마는 패배 연출 중에만 영역 높이가 달라지므로, 그런 테마를 더할 때는 이 점을 감안한다.
