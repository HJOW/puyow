...
- 보유 카드는 `puyow_cards`의 개별 `{id,type}` 인스턴스 배열이다. 기본 카드 풀은 일반뿌요 5종, 방해뿌요 3종, 예고뿌요 8종(단위 1·6·30·210·500·2000·80000·500000), 출시 적 11종(안드로말리우스~발라크)이며 `getCardDefinitions()`의 가중치를 쓴다. 안드라스·발라크를 포함해 플라우로스 뒤에 추가된 출시 적은 자동으로 가중치 1 카드가 되며, 솔로몬과 `notAvail` 출시 예정 적은 제외한다. 단위 500000보다 큰 예고뿌요도 자동으로 가중치 1 카드가 된다. 가중치 경계에 따른 EPIC/RARE/UNCOMMON/COMMON 배경색만 그리며 등급 문구는 카드에 표시하지 않는다.
...
- **세션·추론 수명은 대전 범위로 제한한다**: `onnxSessionCache`는 양쪽 적이 쓰는 최대 2개 모델만 빌릴 수 있으며, 결과 화면을 닫거나 일시정지에서 종료하거나 `destroy()`할 때 각 `OnnxEnemy.releaseModel()`이 마지막 대여 세션의 `release()`를 호출한다. 같은 세션의 `run()`은 `runOnnxSessionIfIdle()`이 하나만 허용하고, 이미 실행 중이면 새 요청을 쌓지 않고 그 턴을 앞 1수 시뮬레이션으로 결정한다. 입력 텐서와 모든 출력 텐서는 `finally`에서 `dispose()`한다. 따라서 시간 초과 뒤 실제 `run()`이 늦게 끝나도 여러 요청·텐서가 누적되지 않는다.
