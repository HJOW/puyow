...
- `Seere`의 피버 비활성 일반 쌓기만 오른쪽 두 열 전체 → X=3의 화면 절반(6칸) → X=0 전체 → X=1 전체 순서다. 피버 비활성 피버 룰의 별도 빌드, 피버 중 공통 연쇄 최적화, 패배 위치 보호, 공격 시뮬레이션의 호출 조건·주기, 빈 필드 무작위 착수는 이 규칙보다 우선하며 변경하지 않는다.
