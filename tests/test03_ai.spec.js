...
/** 모델을 쓰는 첫 적(무르무르)까지 이긴 진행도와 무르무르 갤러리 해금 기록을 저장한다. */
async function seedAllOpponentsCleared(page) {
  await page.evaluate(() => {
    const cleared = ['Andromalius', 'Dantalion', 'Seere', 'Decarabia', 'Belial', 'Amdusias', 'Kimaris', 'Andrealphus', 'Flauros', 'Andras', 'Valak', 'Zagan', 'Vapula', 'Oriax', 'Amii', 'Ose', 'Gremory', 'Orobas', 'Murmur'];
    localStorage.setItem('puyow_store', JSON.stringify({
      clearList: [],
...
test('ONNX 런타임이 없으면 무르무르는 적 선택 화면에서 빠지고 모델을 쓰지 않는 오로바스는 남는다', async ({ page }) => {
  ...
  // 이전 적과 무르무르를 모두 이긴 기록이 있어도 무르무르는 목록에 나오지 않는다. 오로바스는 모델을 쓰지 않으므로 그대로 나온다.
  await expect.poll(() => page.evaluate(() => window.testCanvasTexts.some((text) => ['오로바스', 'Orobas', 'オロバス', '欧罗巴士'].includes(text)))).toBe(true);
  expect(await page.evaluate(() => window.testCanvasTexts.some((text) => ['무르무르', 'Murmur', 'ムルムル', '穆尔穆尔'].includes(text)))).toBe(false);
});

test('ONNX 런타임이 없어도 이긴 전적이 있는 무르무르는 갤러리에서 잠금 해제된다', async ({ page }) => {
  await seedAllOpponentsCleared(page);
  await page.route('**/js/ort.all.min.js', (route) => route.fulfill({ status: 200, contentType: 'text/javascript', body: '' }));
  await page.reload();
  await expect.poll(() => page.evaluate(() => window.WebPuyo.getScreenState().screen)).toBe('initial_title');
  ...
  await page.keyboard.press('ArrowDown');
  await page.keyboard.press('ArrowDown');
  await expect.poll(() => page.evaluate(() => window.testCanvasTexts.some((text) => ['무르무르', 'Murmur', 'ムルムル', '穆尔穆尔'].includes(text)))).toBe(true);
});
...
test('무르무르는 ONNX 모델을 불러온 뒤 대전하고, 모델을 못 불러오면 적 선택 화면으로 돌아간다', async ({ page }) => {
  ...
  // 돌아온 적 선택 화면은 첫 줄에 포커스가 있으므로 다시 적 줄로 내려 시작한다. 선택된 적은 그대로 무르무르다.
  await page.keyboard.press('ArrowDown');
  await page.keyboard.press('ArrowDown');
  await page.keyboard.press('Enter');
  await page.keyboard.press('Enter');
...
test('ONNX 추론 적은 이름 옆에 느낌표를 달고, 처음 시작할 때만 불안정 안내에서 계속을 골라야 대전한다', async ({ page }) => {
  ...
  for (const name of ['발라크', '자간', '바퓰라']) {
    await expect.poll(() => hasMarkBesideName(name, 450)).toBe(false);
    await page.keyboard.press('ArrowRight');
  }
  for (let index = 0; index < 5; index += 1) await page.keyboard.press('ArrowRight');
  for (const name of ['무르무르', '카임', '알로케스']) {
    await expect.poll(() => hasMarkBesideName(name, 513)).toBe(true);
  }
  await expect.poll(() => hasMarkBesideName('무르무르', 450)).toBe(true);
  await page.keyboard.press('ArrowLeft');
  await expect.poll(() => hasMarkBesideName('오로바스', 450)).toBe(false);
  await page.keyboard.press('ArrowRight');
...
test('CDN과 로컬 모두 wasm을 못 받으면 적 선택 화면에서만 무르무르를 숨긴다', async ({ page }) => {
